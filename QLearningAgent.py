...
class QLearningAgent(Board):
    def __init__(
        self, num_states=None, num_actions=None, epsilon=0.1, alpha=0.5, gamma=0.9
    ):
        super().__init__()
    ...
    def init_agent(self, verbose):
        if verbose:
            print("num_states:", 3 ** ((NUM_LINES * NUM_LINES)))
            print("num_actions:", ((NUM_LINES * NUM_LINES)))
            self.verbose = verbose
        self.num_states = 3 ** (NUM_LINES * NUM_LINES)
        self.num_actions = NUM_LINES * NUM_LINES
    ...
    def get_reward(self, process_name: str) -> int:
        ...
        if (
            self.is_runnable_next_process(self.stock, self.process[process_name])
            == False
        ):
            return -100
        if all(
            elem in list(self.process[process_name].result.keys())
            for elem in self.max_optimize_need_stocks
        ):
            return 20
    ...
    def generate_inventory(self, inventory: list, delay: int) -> list:
        ...
        while self.is_optimized() == False:
            ...
            if len(self.walk) != 0:
                ...
                if (
                    self.is_runnable_next_process(stock, self.process[process_name])
                    == False
                ):
                    stock = dict(self.stock)
            ...
            if len(next_process_lst) == 0:
                self.finshed = True
                if (
                    self.max_optimize_process.name != process_name
                    and process_name not in self.get_optimize_process_lst()
                ):
                    self.cycle = 0
                    return None
                else:
            ...
            self.update_q_table(
                state_num, action_num, self.get_reward(process_name), next_state
            )
